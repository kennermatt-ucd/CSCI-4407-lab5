...
# Test 5: modified message2.txt, then entered the original tag → expected: failure
# Test 6: entered the tag with one character changed → expected: failure
```
...
HMAC ensures integrity as any modification to the message not matter the size (can even be a single bit) will give us a totally different tag. HMAC ensures authenticity as the tage is generated using a secret key known onlt to the two parties engaged in communication (sender and reciever), any attacker can not forge the message without the key needed to generate a valid tag. 
...
In every case, the verification script printed "Verification failed: Message has been altered or tag is invalid." This occurred because the HMAC tag is generated from the every single bit of the message. When the original message is altered, the new tag from the modified message no longer matches the original tag. We then identify that the message has been tampered with and we properly reject it

This experiment shops how MACs provide integrity protection; an attacker who intercepts our message and then modifies its contents cannot produce a proper valid tag for the altered message without knowing our secret key. Since our secret key is shared only by those engaging in legitimate communication, a validated tag gives the message receiver acknowledgement that the message was not altered and was written by someone possessing the shared secret key.
...
A MAC only checks whether the tag is correct for the contents of the message using the provided secret key. The verification process has no memory of previous transactions and therefore it does not store what message and key pairs have been processed before. Because the message and tag have not changed between replay attempts the MAC tag still matches the sent tag, so then verification succeeds every time. Due to the verifcation accepting the pairs over and over again the receiver accepts these messages every time without fail. MAC alone cannot gurantee freshness as it needs additional help to determine when the messages were sent and if they have been previously processed.

### Discussion: Preventing Replay Attacks

To prevent replay attacks we need additional information that identifies which messages have been sent when, in what order, and if the message has been sent previously. This is done with Timestamps, Sequence numbers, and nonces; these all help MACs know if the message is being replayed or not. Timestamps allow the MAC to see the time the message was sent and check if it was sent recently or if it has been sent earlier and therefore is outdated then rejected by the system. Sequence numbers show what order the messages have been sent by incrementing the message counter, the reciever tracks this counter and rejects any message that is not higher than the last message sent on the counter. Nonces are a randomly generated value that the reciever generates and can be used to challenge messages that are sent to the reciever. This Nonce is used by the reciever to see if the sender is authentic by seeing if the nonce is included in the next message, as all nonces are generated per session an old nonce will not be the same as the new nonce and a replayed message containing an old unsed nonce will then have their message rejected.
